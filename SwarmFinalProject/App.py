...
st.set_page_config(page_title="Swarm Bot", layout="wide")
...
[class*="st-key-msg_"] {{
    ...
    border: 1px solid #E5D3A6 !important;
...
[class*="st-key-msg_"][class*="_assistant"] {{
    background-color: #FBEFD3 !important;
    border-color: #E5D3A6 !important;
    border-radius: 16px !important;
    padding: 8px 14px !important;
    color: #4A3B25 !important;
...
.ask-bar [data-testid="stFormSubmitButton"] button {{
    ...
    background-color: #4A3B25 !important;
...
.ask-bar [data-testid="stFormSubmitButton"] button:hover {{
    background-color: #6A5738 !important;
...
.ask-bar [data-testid="stFileUploader"] section > div {{
    display: flex !important;
    align-items: center !important;
    gap: 12px !important;
}}

.ask-bar [data-testid="stFileUploader"] button {{
...
.ask-bar [data-testid="stFileUploaderDropzoneInstructions"] > div {{
    margin: 0 !important;
}}

.ask-bar [data-testid="stFileUploader"] svg {{
...
with st.form("ask_form", clear_on_submit=True, border=False):
    ...
    with text_col:
        prompt = st.text_input(
            "Ask anything", placeholder="Ask anything", label_visibility="collapsed"
        )
...
if submitted and prompt:
    if uploaded_file:
        raw_text = extract_text_from_pdf(uploaded_file) if uploaded_file.name.endswith(".pdf") else uploaded_file.read().decode("utf-8")
        chunks = chunk_text(raw_text)
        collection.upsert(documents=chunks, ids=[f"chunk_{i}" for i in range(len(chunks))])

    st.session_state.messages.append({"role": "user", "content": prompt})
    st.session_state.thinking = True
    st.rerun()

if st.session_state.thinking and st.session_state.messages and st.session_state.messages[-1]["role"] == "user":
    prompt = st.session_state.messages[-1]["content"]

    document_context = "\n\n".join(collection.get(include=["documents"])["documents"]) if collection.count() > 0 else ""
    task = f"USER GOAL:\n{prompt}\n\nREFERENCE KNOWLEDGE:\n{document_context}"
    ...
    with ThreadPoolExecutor(max_workers=3) as executor:
        ...
        while futures:
            done_keys = [key for key, future in futures.items() if future.done()]
...
"""
    ...
    diagram_placeholder.markdown(render_diagram(), unsafe_allow_html=True)
    complete_placeholder.markdown("<div class='complete-badge'>✅ COMPLETE</div>", unsafe_allow_html=True)
